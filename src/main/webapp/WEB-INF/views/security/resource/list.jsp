			...
			<table class="table table-striped ">
					<thead>
						<tr>
							...
							<th class="sortable"><a href="#">URL链接</a></th>
							<th class="sortable"><a href="#">优先级</a></th>
						    ...
						    <c:choose>
								<c:when test="${not empty page.result}">
									<c:forEach items="${page.result}" var="resource">
										<tr class="table_con">
											<td><input type="checkbox" name="items" id="checkbox" value="${resource.id}"/></td>
											<td>${resource.name}&nbsp;</td>
											...
											<td  class="link">
												<se:authorize ifAnyGranted="A_SECURITY_RESOURCE_EDIT">
													<a class="btn btn-small" href="${ctx }/security/resource/${resource.id}/edit"><i class="icon-pencil"></i> 编辑</a>
													<a class="btn btn-small" href="javascript:;"  onclick="deleteRow('${resource.id}')" ><i class="icon-remove"></i> 删除</a>
												</se:authorize>
												&nbsp;	
											</td>
										</tr>
...
<script type="text/javascript">
	$(function(){
		...
		$("#add").click(function(){
			location.href = "${ctx}/security/resource/create";
		});
		...
		$("#deleteId").click(function(){
			...
			if(items && items.length > 0){
				if(confirm("确定要清空数据吗？")){
					$("#myForm").attr("action", "${ctx}/security/resource/delete")
								.attr("method","post")
	...
	function deleteRow(value){
		if(confirm("确定要清空数据吗？")){
			$("#myForm").attr("action", "${ctx}/security/resource/"+value+"/delete")
				.attr("method","post")
				.append('<input type="hidden" name="_method" value="DELETE" />')
				.submit();
		}
